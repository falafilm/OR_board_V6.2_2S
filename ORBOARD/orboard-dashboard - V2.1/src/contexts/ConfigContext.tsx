...
const normalizeConfig = (
  rawConfig: Partial<ConfigData> | null | undefined,
): ConfigData => {
  const parsedConfig = rawConfig || {};
  const fallbackCount = parseInt(
    parsedConfig.count_per_cycle?.toString() || "1",
  );
  const normalizedCount = Number.isNaN(fallbackCount) ? 1 : fallbackCount;
  const parsedPlan = parseInt(
    parsedConfig.count_per_cycle_plan?.toString() || normalizedCount.toString(),
  );
  const parsedActual = parseInt(
    parsedConfig.count_per_cycle_actual?.toString() ||
  ...
  return {
    ...defaultConfig,
    ...parsedConfig,
    count_per_cycle: normalizedCount,
    count_per_cycle_plan: Number.isNaN(parsedPlan)
      ? normalizedCount
      : parsedPlan,
    count_per_cycle_actual: Number.isNaN(parsedActual)
      ? normalizedCount
      : parsedActual,
  };
};
...
export function ConfigProvider({ children }: { children: ReactNode }) {
  ...
  const [config, setConfig] = useState<ConfigData>(() => {
    ...
    return { ...normalizeConfig(parsed), model };
  });
  ...
  const fetchConfig = async () => {
    try {
      setIsLoading(true);
      const liveResponse = await fetch(`${API_BASE_URL}/live`);
      ...
      const apiLine = liveData?.line?.toString()?.trim();

      if (!apiLine) {
        throw new Error("Live API response did not include a line");
      }

      const response = await fetch(`${API_BASE_URL}/${apiLine}`);
